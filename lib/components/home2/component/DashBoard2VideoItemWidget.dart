  ...
  Widget build(BuildContext context) {
    Widget imageWidget() {
      if (videoData.image_url.validate().isNotEmpty) {
        return cachedImage(videoData.image_url.validate(), fit: BoxFit.cover).cornerRadiusWithClipRRect(defaultRadius - 4);
      } else if (videoData.video_type.validate() == VideoTypeYouTube) {
        return cachedImage(videoData.video_url.validate().getYouTubeThumbnail(), fit: BoxFit.cover).cornerRadiusWithClipRRect(defaultRadius - 4);
      }
    ...
    return Container(
      ...
      child: Stack(
        ...
        children: [
          imageWidget(),
          Container(color: Colors.black38).cornerRadiusWithClipRRect(defaultRadius - 4),
          Positioned(
